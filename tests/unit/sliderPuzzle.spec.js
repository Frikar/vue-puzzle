...
describe('SliderPuzzle.vue', () => {
    ...
    it('stops timer when sstop metho is called', () => {
        const wrapper = mount(SliderPuzzle)
        wrapper.vm.stop();
    ...
    it('records record to local storage', () => {
        ...
        expect(localStorage.setItem).toHaveBeenLastCalledWith('records', stringifiedRecords);
    })
        ...
        const wrapper = mount(SliderPuzzle, {
            data() {
                return {
                    ...
                    startDateTime: new Date(2021, 0, 0, 0, 0, 0)
                }
            }
        })
